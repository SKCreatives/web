...
function statLikeDropbox(err, stat, filepath, filename) {
  if (arguments.length < 3) {
    throw new Error('statLikeDropbox(err, stat, filepath<, filename>) missing arguments');
  }
...
function Storage(storageURI, options) {
  ...
  if (!storageURI) {
    throw new Error('storageURI missing');
  }
  ...
  switch (this.protocol) {
    case "dropbox":
      if (!credentials) {
        throw new Error('Storage missing Dropbox credentials');
      }
      util.log('Using Dropbox storage');
      this.rootRegExp = new RegExp('^' + this.root, 'i');
      this.dropboxClient = this.dropboxClient || new Dropbox.Client({
    ...
    default:
      if (!this.appRoot) {
        throw new Error('Storage missing app root in FS mode');
      }
      util.log('Using FS storage');
...
Storage.prototype.readdir = function(pathname, options, callback) {
  if (!pathname.match(this.rootRegExp)) {
...
Storage.prototype.writeFile = function(filename, data, options, callback) {
  if (!filename.match(this.rootRegExp)) {
