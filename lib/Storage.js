...
function Storage(storageURI, options) {
  ...
  switch (this.protocol) {
    case "dropbox":
      if (!options.credentials) throw new Error('Storage missing Dropbox credentials');
      this.rootRegExp = new RegExp('^' + this.root, 'i');
      this.dropboxClient = new Dropbox.Client({
        key: options.credentials.DROPBOX_APP_KEY,
        secret: options.credentials.DROPBOX_APP_SECRET,
        token: options.credentials.DROPBOX_TOKEN
      });
      break;
