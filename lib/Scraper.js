...
Project.prototype.resolveURIAsync = function(callback) {
  ...
  request.head({uri: this.uri, timeout: 3000 }, _.bind(function(err, res) {
    if (!err) {
...
Project.prototype.getAsync = function(callback) {
  var start = Date.now();
  callback = callback || this.__NOOP;
  request.get({uri:this.uriResolved}, function(err, res, body) {
    if (err) {
...
Project.prototype.loadFromCacheAsync = function(callback) {
  ...
  if (hash) {
    fs.readFile(this.cacheDir + '/' + hash + '.json', function(err, data) {
      var JSONdata;
      if (!err) {
    ...
    // We have no hash, so search ALL files in cache
    fs.readdir(this.cacheDir, function(err, files) {
      async.detect(files, function(file, callback) {
        fs.stat(self.cacheDir + '/' + file, function(err, stats) {
          var found, JSONdata;
          if (stats.isFile() && !file.match(re)) {
            JSONdata = require(self.cacheDir + '/' + file);
            found = (JSONdata.uri === self.uri) ? true : false;
          }
          callback(found);
        });
      }, function(result) {
        var JSONdata;
        if (result) {
           JSONdata = require(self.cacheDir + '/' + result);
        }
        done(JSONdata);
      });
    });
...
Project.prototype.cacheToDiskAsync = function(callback) {
  ...
  fs.writeFile(__dirname + '/../data/cache' + '/' + hash + '.json', json, function (err) {
...
// Prototype RegExpes
Project.prototype.__URIProtocolRegExp = new RegExp('https?://', 'gi');
Project.prototype.__IGIDRegExp = new RegExp('indiegogo.com/projects/(\\d+)', 'gi');
Project.prototype.__IGTimeRegExp = new RegExp('\\((\\d+:\\d+.*)PT\\)', 'gi');
Project.prototype.__IGDateRegExp = new RegExp('\\s(\\w+\\s\\d+,\\s\\d+)\\s', 'gi');
Project.prototype.__KSProjectJSONRegExp = new RegExp('\\.current_project = "(.*)"', 'gi');
...
var Scraper = function(options) {
  ...
  _.assign(this, options, this);

  // Init
  _.each(this.projects, function(project, i) {
    this.projects[i] = new Project(project);
  }, this);
};
...
Scraper.prototype.start = function() {
  ...
  async.eachSeries(this.projects, function(project, next) {
    self.updateProjectAsync(project, function() {
